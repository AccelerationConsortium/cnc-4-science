...
## Unreleased
- **Repo housekeeping**: `CHANGELOG.md`, `README_PYPI.md`, and `LICENSE` moved into [`docs/`](.) so the top-level tree only carries the GitHub-landing `README.md` and source dirs. `pyproject.toml` `readme = "docs/README_PYPI.md"`, `license-files = ["docs/LICENSE"]`, and the `Changelog` URL were updated; root README's license link likewise. [`examples/AGENTS.md`](../examples/AGENTS.md) stays under `examples/` so Copilot/Codex/Cursor auto-discover the scaffolding conventions from inside an example folder.
- **Pick-and-place: storage + gameboard 3D-printed racks added.** New parts `storage-15-tuberack-100ul.{step,stl}` (deck slot 3, back-left, X/O reservoir) and `gameboard-15-tuberack-100ul.{step,stl}` (deck slot 4, back-right, 3\u00d73 play area) shipped under [`examples/vacuum_pick_and_place/cad/`](../examples/vacuum_pick_and_place/cad/). Filenames mirror the matching Opentrons `loadName`s (`storage_15_tuberack_100ul.json`, `gameboard_15_tuberack_100ul.json`) so JSON \u2194 CAD pairing is obvious. BOM (rows B6/B7) + ASSEMBLY_INSTRUCTIONS Stage 2 wording + README deck-layout table all updated to reflect slots 3/4 (matching what `tools/cnc_config.yaml` already declared \u2014 the README was the stale doc).

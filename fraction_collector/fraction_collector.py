...
DECK_DEF_PATH = os.path.join(os.path.dirname(__file__), "deck", "cnc_deck_definition.json")
...
class FractionCollector:
    ...
    def load_tool(self, tool_config):
        ...
        if not cls:
            raise ValueError(f"Unknown tool type '{tool_id}'. Available: {list(TOOL_CLASSES.keys())}")
    ...
    def get_tool(self, tool_id):
        ...
        if tool_id not in self.tools:
            raise KeyError(f"Tool '{tool_id}' not loaded. Available: {list(self.tools.keys())}")

...
- **Hardware docs (placeholders)**: new [`docs/hardware/`](docs/hardware/) directory with `README.md`, `BOM.md`, `ASSEMBLY.md`, `CAD.md` stubs for the upcoming hardware-build documentation (bill of materials, assembly steps, CAD / 3D-print index).
- **Examples index**: new [`examples/README.md`](examples/README.md) summarizing each example project; new [`examples/AGENTS.md`](examples/AGENTS.md) documenting the shared layout, config schema, and driver-class pattern used across `hello_cnc/`, `liquid_handling/`, and `vacuum_pick_and_place/`.
- **README**: added links to the hardware docs and examples index.
